...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		for ( var i = 1; i < parseInt($('#iterator').html()); i++) {
			var img = $('.other' + "_" + i);
			$(img).each(function() {
		...
		$('#previews').find('img:last').load(function() {
                    ...
                    <c:otherwise>
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:choose>
                                    <c:when test="${(instanceNumber == middle) || (instanceNumber==1) || (instanceNumber==numberOfImages)}">
                                        <img name="${instanceNumber}" class="image" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize}" src="Image.do?serverURL=${param.wadoUrl}&study=${param.study}&series=${seriesId}&object=${imageId}&retrieveType=${param.wadoUrl}&sopClassUID=${sopClassUID}&seriesDesc=${seriesDesc}&images=${numberOfImages}&modality=${modality}" onclick="changeSeries(this)" />
                                    </c:when>
                                    <c:otherwise>
                                        <img name="${instanceNumber}" class="image" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize} display: none;" src="Image.do?serverURL=${param.wadoUrl}&study=${param.study}&series=${seriesId}&object=${imageId}&retrieveType=${param.wadoUrl}&sopClassUID=${sopClassUID}&seriesDesc=${seriesDesc}&images=${numberOfImages}&modality=${modality}" onclick="changeSeries(this)" />
                                    </c:otherwise>
                                </c:choose>
                            </c:otherwise>
                        </c:choose>
