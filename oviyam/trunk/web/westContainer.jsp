        ...
        <script type="text/javascript">        	
            ...
            $(document).ready(function() { 	              	
            	...
            	lastImg.load(function() {
       		 		var selectedStudy = jQuery(jQuery('.accordion').get(0)).attr('id');
            		storeSer(selectedStudy,JSON.parse(sessionStorage[selectedStudy]));
           		});  
           		
           		var studies = jQuery('.accordion');
           		for(var i=1;i<studies.length;i++) {
           			acc(jQuery(studies[i]));
           		}           		
            });  //document.ready          
            ...
            function showFirstImage(imgTag,insNo) {            	
            	if(!wlApplied) {
        			jQuery(imgTag).click();
            	}
            }   
...
<!--	        <table id="studyTable" style="font-family: Arial; font-size:13px; width: 100%;color: #FF8A00; padding-bottom:3px; border: none;">
        ...
        <div id="previews" style="overflow: auto; height: 90%;">
        	...
        	<stu:Study patientId="${param.patient}" study="${param.study}" dcmURL="${param.dcmURL}">        			    		   		
        	
        		<script type="text/javascript">
	        		series = new Array();
        		</script>        		

					<div id='${studyId}' class="accordion open" title="${tooltip}" onclick="acc(jQuery(this));">${dateDesc}</div>
        			<div>
		    		<ser:Series patientId="${param.patient}" study="${studyId}" dcmURL="${param.dcmURL}">
													...
													<td colspan="2">
														...
														<c:choose>
															<c:when test="${modality == 'SR'}"> <!-- Structured Report -->
								                   				 <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="height: 75px;" src="images/SR_Latest.png" class="image" imgSrc="Image.do?serverURL=${param.wadoUrl}&study=${studyId}&series=${seriesId}&object=${imageId}&sopClassUID=${sopClassUID}"  onclick="changeSeries(this);" onload="if(${selected} && ${instanceNumber}==1 && ${firstSeries}){showFirstImage(this,${instanceNumber});}" />
								               				 </c:when>
								                
								               				<c:when test="${sopClassUID == '1.2.840.10008.5.1.4.1.1.104.1'}"> <!-- Enacpsulated PDF -->
								                    			<img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize}" src="images/pdf.png" class="image" imgSrc="Image.do?serverURL=${param.wadoUrl}&study=${studyId}&series=${seriesId}&object=${imageId}&sopClassUID=${sopClassUID}"  onclick="changeSeries(this)" onload="if(${selected} && ${instanceNumber}==1 && ${firstSeries}){showFirstImage(this,${instanceNumber});}" />
								               				 </c:when>
								                
								                			<c:when test="${fn:contains(sopClassUID,'1.2.840.10008.5.1.4.1.1.9')}"> <!-- Wave Forms -->
														        <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize}" src="images/pdf.png" class="image" imgSrc="Image.do?serverURL=${param.wadoUrl}&study=${studyId}&series=${seriesId}&object=${imageId}&sopClassUID=${sopClassUID}&rid=true"  onclick="changeSeries(this)" onload="if(${selected} && ${instanceNumber}==1 && ${firstSeries}){showFirstImage(this,${instanceNumber});}" />
														    </c:when>
								                
								                			<c:when test="${sopClassUID == '1.2.840.10008.5.1.4.1.1.66'}"> <!-- Raw Data Storage -->
														        <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize}" src="images/rawdata.png" onclick="showRawDataAlert();" onload="if(${instanceNumber}==1 && ${firstSeries}){showFirstImage(this,${instanceNumber});}" />
														    </c:when>
														    ...
														    <c:otherwise>	<!-- Normal images -->
														    	<c:choose>	
														    		...
														    		<c:otherwise>
														    			<c:choose>
														    				<c:when test="${(instanceNumber == middle) || (instanceNumber==1) || (instanceNumber==numberOfImages)}">
														    					<img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" style="${thumbSize}" class="image" src="Image.do?serverURL=${param.wadoUrl}&study=${studyId}&series=${seriesId}&object=${imageId}&sopClassUID=${sopClassUID}&seriesDesc=${seriesDesc}&images=${numberOfImages}&modality=${modality}"  onclick="changeSeries(this)" onload="if(${selected} && ${instanceNumber}==1 && ${firstSeries}){showFirstImage(this,${instanceNumber});}"/>
														    				</c:when> 
														    				<c:otherwise>	
